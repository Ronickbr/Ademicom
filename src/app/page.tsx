...
export default function Home() {
  const [stats, setStats] = useState<DashboardStat[]>([
    ...
    { name: "Pendente Liberação", value: "0", icon: AlertCircle, color: "text-orange-500", key: "TECNICO", trend: "+8%", label: "Em Análise" },
    { name: "Total Expedido", value: "0", icon: CheckCircle2, color: "text-emerald-500", key: "LIBERADO", trend: "+24%", label: "Concluídos" },
  ]);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const queries = [
          ...
          supabase.from("products").select("*", { count: 'exact', head: true }).eq('status', 'TECNICO'), // Pendente
          supabase.from("products").select("*", { count: 'exact', head: true }).eq('status', 'LIBERADO'), // Concluido
      ];
    ...
    <div className="space-y-6 pb-20 sm:pb-0">
      ...
      <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="glass-card p-4 sm:p-6 bg-neutral-900/40 border-white/5 relative overflow-hidden group hover:border-primary/30 transition-all"
          >
